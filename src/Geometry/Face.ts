...
export class Face extends GeometryBase {
    ...
    public getAllPointsInside() {
        const facePoints = this.getPoints();
        const maxPoint = Point.max(...facePoints).ceil();
        const minPoint = Point.min(...facePoints).floor();
        ...
        for (let x = minPoint.x; x <= maxPoint.x; x++) {
            const line = new Line(new Point(x, minPoint.y), new Point(x, maxPoint.y));
            let intersections = [];
            lines.forEach(l => {
            ...
            intersections.forEach((intersection) => {
                if (previousIntersection && intersection.y > previousIntersection.y) {
                    ...
                    const lineCenter = new Line(intersection, previousIntersection).getCenter();
